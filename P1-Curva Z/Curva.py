...
#======================================
# Sosa Mendoza Rodrigo ESFM IPN 2023
#======================================
import numpy as np
...
class Curva:
    ...
    def interpolacion(s , p:int=0 , r:float=0.0) -> list:
        ...
        #===========================
            ...
            for j in range(1, s.dim):
                xi.append(zp1 * s.x[ip1 + j * s.n] + z * s.x[i + j * s.n] + zp2 * s.x[ip2 + j * s.n] + zm1 * s.x[
                    im1 + j * s.n])
        ...
        #=============================
            ...
            for j in range(1, s.dim):
                xi.append(zp1 * s.x[ip1 + j * s.n] + z * s.x[i + j * s.n] + zp2 * s.x[ip2 + j * s.n] + zp3 * s.x[
                    ip3 + j * s.n] + zm1 * s.x[im1 + j * s.n] + zm2 * s.x[im2 + j * s.n])
...
#====================================================================================
# Funcion zspline  crea eñ Z-spline de un conjunto de puntos en dim dimensiones
# n segmentos y continuidad (aproximacion) cot
# x,y = zspline(conjunto de puntos, dimenison, numeros de segementos, continuidad)
#====================================================================================
