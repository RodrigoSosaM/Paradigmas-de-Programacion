...
@jit
def evolucion(u, n_0 , n_1 , udx2_0 , dt , kd , i):
    jpl = i +n_0
    jml = i -n_0
    laplaciano = (u[i-1]-2.0*u[i]+u[i+1])*udx2[0]+(u[jml]-2.0*u[i]+u[jpl])*udx2_1
...
@cuda.jit
def solucion_kernel(u_d , un_d , udx2_1 , dt , n_0 , n_1 , kd):
    ii , jj = cuda.grid(2)
    i  = ii + n_0*jj
    if ii==0 or jj==0 or ii== n_0-1 or jj ==n_1-1:
        unueva = 0.0
    else:
        unueva = evolucion_gpu(u_d , n_0 , n_1 , udx2_1 , dt , kd , i)
...
#===================================
# Pasar arreglos al evolucion_gpu
#===================================
...
for t in range(1 , pasos+1):
    solucion_kernel[girddim , blockdim](u_d , un_d , udx2[0] , udx2[1] , dt , n[0] , n[1] , kd)
    u_d = cuda.to_divice(un_d)
